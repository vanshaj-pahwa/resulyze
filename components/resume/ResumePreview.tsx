  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardContent className="p-8">
        <div className="resume-preview bg-white text-black" style={{ fontFamily: 'Arial, sans-serif', fontSize: '11px', lineHeight: '1.2' }}>
          {/* Header */}
            ...
            {resumeData.workExperience.map((exp: any, index: number) => (
              <div key={index} className="mb-3">
                <div className="flex justify-between items-start mb-1">
                  <div>
                    <strong className="text-xs">{exp.title}</strong> | {exp.company}
                  </div>
                  <div className="text-xs">{exp.startDate} - {exp.current ? 'Present' : exp.endDate}</div>
                </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex justify-between items-start mb-1">
              <div className="text-xs">
                <strong>{resumeData.education.degree}</strong> | {resumeData.education.institution}
              </div>
              <div className="text-xs">{resumeData.education.graduationDate}</div>
            </div>
            <div className="text-xs italic">{resumeData.education.location} | GPA: {resumeData.education.gpa}</div>
          </div>
          ...
          {/* Achievements */}
          <div className="mb-4">
            <h2 className="text-sm font-bold mb-2">ACHIEVEMENTS AND CERTIFICATIONS</h2>
            <ul className="text-xs space-y-1">
              {resumeData.achievements.map((achievement: string, index: number) => (
                <li key={index}><strong>{achievement.split(':')[0]}:</strong> {achievement.split(':').slice(1).join(':')}</li>
              ))}
            </ul>
          </div>
        </div>
